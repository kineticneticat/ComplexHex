...
class ParametricLineRenderer(val context: EntityRendererProvider.Context): EntityRenderer<ParameticLineEntity>(context) {
    val tex: ResourceLocation = id("textures/entity/holdout.png");

    override fun getTextureLocation(entity: ParameticLineEntity) = tex

    override fun render(para: ParameticLineEntity, f: Float, g: Float, poseStack: PoseStack, multiBufferSource: MultiBufferSource, i: Int) {
        val steps = 100.0
        var start: Vec3
        var end: Vec3
        for (t in 0 until steps.toInt()) {
            start = getPosAtT(para, t/steps) ?: Vec3(t/steps, t/steps, t/steps)
            end = getPosAtT(para, (t+1)/steps) ?: Vec3((t+1)/steps, (t+1)/steps, (t+1)/steps)
